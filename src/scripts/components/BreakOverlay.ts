...
export function createBreakOverlay(props: BreakOverlayProps): HTMLElement {
  ...
  overlay.setAttribute('aria-labelledby', 'break-overlay-title')
...
export function showBreakOverlay(): void {
  ...
  if (overlay) {
    ...
    const card = overlay.querySelector('.stat-card-glass') as HTMLElement
    const cancelBtn = overlay.querySelector('button') as HTMLElement
    modalCleanup?.()
    if (card) {
      modalCleanup = setupModal(overlay, card, {
        onEscape: () => cancelBtn?.click(),
