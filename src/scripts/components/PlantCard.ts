...
function createPlantVisual(plant: Plant): HTMLElement {
  const def = getPlantDefinition(plant.type)
  const visual = document.createElement('div')
  visual.className =
    'w-full h-full flex items-center justify-center bg-gradient-to-br from-primary/10 to-primary-container/20 text-6xl md:text-7xl'
  visual.setAttribute('aria-hidden', 'true')
  visual.textContent = def?.emoji ?? '🌱'
  return visual
}
...
export function createPlantCard(props: PlantCardProps): HTMLElement {
  ...
  const def = getPlantDefinition(plant.type)
  const displayName = def?.name ?? plant.type
  ...
  if (isFeatured) {
    ...
    imgWrap.appendChild(createPlantVisual(plant))
  ...
  date.textContent = new Date(plant.plantedAt).toLocaleDateString()
