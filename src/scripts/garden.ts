...
async function initGardenPage() {
  setPageMeta({
    title: 'My Garden',
    description: 'Grow plants by completing focus sessions. Choose seeds and track your garden progress.',
    path: 'garden.html',
  })
  ...
  if (gridContainer) {
    ...
    if (!activePlant) {
      ...
      emptyCard.className =
        'rounded-2xl border-2 border-dashed border-outline-variant/50 flex flex-col items-center justify-center gap-2 text-on-surface/50 hover:text-primary hover:border-primary/50 transition-all duration-200 cursor-pointer aspect-square p-4'
      emptyCard.innerHTML = `
        <span class="material-symbols-outlined text-3xl">add</span>
        <span class="font-label text-sm font-semibold">New Sprout</span>
        <span class="font-body text-[10px] text-on-surface/40 text-center">Your garden is empty — pick a plant to start growing</span>
      `
...
bootstrapPage(initGardenPage).catch(console.error)
